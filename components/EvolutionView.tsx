...
import { BarChart3, TrendingUp, Calendar, Filter, Zap, Activity, ChevronDown, Clock } from 'lucide-react';
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      <div className="bg-slate-900 rounded-[3rem] p-10 lg:p-14 text-white shadow-2xl relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-red-600/10 rounded-full blur-[80px] -ml-20 -mb-20"></div>
           ...
           {timeScale !== 'years' && (
              <div className="relative group">
                <select 
                  ...
                  className="bg-slate-50 border border-slate-200 rounded-xl px-6 py-3 text-[10px] font-black uppercase tracking-widest text-slate-700 outline-none focus:ring-2 ring-blue-500 appearance-none pr-10 cursor-pointer"
                >
           ...
           {timeScale === 'days' && (
              <div className="relative group">
                <select 
                  ...
                  className="bg-blue-600 text-white border border-blue-500 rounded-xl px-6 py-3 text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 ring-blue-400 appearance-none pr-10 cursor-pointer shadow-lg shadow-blue-100"
                >
          ...
          <ResponsiveContainer width="100%" height="100%">
            {activeData.length === 0 ? (
              <div className="flex items-center justify-center h-full flex-col gap-4">
                ...
                <p className="text-xs font-black text-slate-300 uppercase tracking-widest">Aucune donnée pour cette période</p>
              </div>
            ) : timeScale === 'days' ? (
              <AreaChart data={activeData}>
                ...
                />
                <Area type="monotone" dataKey="total" stroke={COLORS.blue} fillOpacity={1} fill="url(#colorTotal)" strokeWidth={4} name="Total Poches" />
              ...
              <BarChart data={activeData}>
                ...
                <Line type="monotone" dataKey="total" stroke={COLORS.blue} strokeWidth={3} dot={{ r: 4, fill: COLORS.blue, strokeWidth: 2, stroke: '#fff' }} name="Cumul" />
              </BarChart>
            )}
          </ResponsiveContainer>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          ...
          <div key={i} className="bg-white p-10 rounded-[3rem] shadow-warm border border-orange-50 flex items-center justify-between transition-all hover:scale-[1.02]">
            <div className="flex items-center gap-6">
               <div className={`w-14 h-14 ${item.light} ${item.text} rounded-2xl flex items-center justify-center shadow-inner`}>{item.icon}</div>
               <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{item.label}</p>
                  <p className="text-4xl font-black text-slate-900 tracking-tighter">{item.value.toLocaleString()}</p>
                  {item.date && (
                    <div className="mt-1">
                      <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none">{item.date}</p>
                      {item.sub && <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-0.5 italic">Site: {item.sub}</p>}
                    </div>
                  )}
               </div>
            </div>
