...
import { TrendingUp, Calendar, Building2, Truck, Award, Target, Zap, Activity, MapPin, User, Phone, Mail, ChevronRight, ArrowUpRight, ArrowDownRight, PieChart, BarChart3, Clock, MessageSquare, CheckCircle2, Filter, AlertCircle, Share2 } from 'lucide-react';
...
export const VisualDashboard: React.FC<{ data: DashboardData }> = ({ data }) => {
  ...
  const handleWhatsAppReminder = (site: any) => {
    ...
    if (cleanPhone.length === 10 && cleanPhone.startsWith('0')) cleanPhone = '225' + cleanPhone;
    else if (cleanPhone.length === 10 && !cleanPhone.startsWith('225')) cleanPhone = '225' + cleanPhone;
    else if (cleanPhone.length === 8) cleanPhone = '22507' + cleanPhone;
    else if (!cleanPhone.startsWith('225')) cleanPhone = '225' + cleanPhone;

    const message = `Bonjour ${site.manager || 'Responsable'}, sauf erreur de notre part, les données de prélèvements du ${selectedDate} pour le site ${site.name} n'ont pas encore été reçues. Pourriez-vous procéder à la saisie dès que possible ? Merci.\n\nCordialement,\nLa Direction DSD CNTSCI.`;
  ...
  return (
    <div className="space-y-8 pb-24 animate-in fade-in duration-700">
      ...
      {/* 1. HUB OPÉRATIONNEL PRIORITAIRE (Ancienne Vue JOUR mise en avant) */}
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-8">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <select 
                      ...
                      className="bg-transparent outline-none text-[10px] font-black uppercase tracking-widest cursor-pointer text-white/60 hover:text-white transition-colors"
                    >
              ...
              <div className="text-right">
                 <p className="text-4xl font-black text-red-500 leading-none">{dayAchievement.toFixed(0)}%</p>
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
               <div className="bg-white/5 p-4 rounded-3xl border border-white/5 text-center">
                  <p className="text-[8px] font-black text-white/30 uppercase mb-1">Fixe</p>
                  <p className="text-xl font-black text-emerald-400">{dailyTotals.fixed}</p>
               </div>
               <div className="bg-white/5 p-4 rounded-3xl border border-white/5 text-center">
                  <p className="text-[8px] font-black text-white/30 uppercase mb-1">Mobile</p>
                  <p className="text-xl font-black text-orange-400">{dailyTotals.mobile}</p>
               </div>
               <div className="bg-red-600/20 p-4 rounded-3xl border border-red-600/20 text-center">
                  <p className="text-[8px] font-black text-red-400 uppercase mb-1">Total</p>
                  <p className="text-xl font-black text-white">{dailyTotals.total}</p>
               </div>
            </div>
            ...
            <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden">
               <div 
                ...
                style={{ width: `${Math.min(dayAchievement, 100)}%` }}
               />
            </div>
          </div>
        </div>

        {/* STATS RAPIDES (MIXITÉ & LEADER) */}
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
           ...
           <div className="bg-white rounded-[3rem] p-8 shadow-warm border border-slate-100 relative overflow-hidden group">
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2 relative z-10">
                <Award size={16} className="text-blue-500" /> Leader {data.month}
              </p>
              <div className="relative z-10">
                 <h4 className="text-2xl font-black text-slate-900 uppercase tracking-tighter leading-tight mb-1">
                   {stats.leader?.name.replace('CRTS ', '').replace('CDTS ', '')}
                 </h4>
      ...
      {/* 2. TRACKING DES TRANSMISSIONS (LIVE FEED) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        
        {/* PANNEAU : EN ATTENTE (RELANCES) */}
        <div className="bg-white rounded-[3.5rem] shadow-warm border border-slate-100 flex flex-col overflow-hidden h-[500px]">
          <div className="p-8 border-b border-slate-50 bg-red-50/30 flex justify-between items-center">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-red-600 text-white rounded-xl flex items-center justify-center shadow-lg shadow-red-100">
                <Clock size={20} />
              </div>
              <div>
                <h3 className="font-black text-lg uppercase tracking-tight text-slate-800">Attente de Saisie</h3>
                <p className="text-[9px] font-black text-red-400 uppercase tracking-widest">{missingSites.length} structures manquantes</p>
              </div>
            </div>
            ...
            {missingSites.length > 0 ? missingSites.map((site, idx) => (
              <div key={idx} className="p-4 bg-slate-50 rounded-3xl border border-slate-100 hover:bg-white hover:shadow-lg transition-all flex items-center justify-between group">
                 <div className="flex items-center gap-4 min-w-0">
                    <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-300 group-hover:text-red-500 transition-colors">
                      <Building2 size={18} />
                    </div>
                    <div className="truncate">
                       <p className="text-[11px] font-black text-slate-800 uppercase truncate">{site.name}</p>
                       <p className="text-[9px] font-bold text-slate-400 uppercase truncate">{site.manager || "Non assigné"}</p>
                    </div>
                 </div>
                 <div className="flex gap-2">
                    {site.phone ? (
                      <button 
                        ...
                        className="w-10 h-10 bg-emerald-500 text-white rounded-xl flex items-center justify-center hover:bg-emerald-600 transition-all shadow-md shadow-emerald-100 active:scale-95"
              ...
              <div className="h-full flex flex-col items-center justify-center text-center py-20 gap-4 opacity-30">
                ...
                <p className="text-xs font-black uppercase tracking-[0.2em]">Toutes les saisies sont à jour</p>
              </div>
            )}
          </div>
        </div>

        {/* PANNEAU : SAISIES DU JOUR VALIDÉES */}
        <div className="bg-white rounded-[3.5rem] shadow-warm border border-slate-100 flex flex-col overflow-hidden h-[500px]">
          <div className="p-8 border-b border-slate-50 bg-emerald-50/30 flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-black text-lg uppercase tracking-tight text-slate-800">Saisies Validées</h3>
                <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">{activeSites.length} transmissions reçues</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 lg:p-8 space-y-3 no-scrollbar">
            {activeSites.length > 0 ? activeSites.sort((a,b) => b.total - a.total).map((site, idx) => (
              <div key={idx} className="p-4 bg-slate-50 rounded-3xl border border-slate-100 hover:bg-white hover:shadow-lg transition-all flex items-center justify-between group">
                 <div className="flex items-center gap-4 min-w-0">
                    <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-emerald-500 transition-colors shadow-sm">
                      <Zap size={18} />
                    </div>
                    <div className="truncate">
                       <p className="text-[11px] font-black text-slate-800 uppercase truncate">{site.name}</p>
                       <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[8px] font-black text-slate-400 uppercase tracking-tighter">{site.region}</span>
                          <span className={`text-[9px] font-black ${site.total >= site.objective ? 'text-emerald-600' : 'text-slate-400'}`}>
                            {((site.total / (site.objective || 1)) * 100).toFixed(0)}% de l'objectif
                          </span>
                       </div>
                    </div>
                 </div>
                 <div className="text-right">
                    <p className="text-lg font-black text-slate-900">{site.total}</p>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center py-20 gap-4 opacity-30">
                ...
                <p className="text-xs font-black uppercase tracking-[0.2em]">En attente du premier flux...</p>
              </div>
            )}
          </div>
        </div>
      ...
      {/* 3. VISION STRATÉGIQUE (Ancienne Vue STATS en support) */}
      <div className="bg-white rounded-[3.5rem] p-10 lg:p-14 shadow-warm border border-slate-100 relative overflow-hidden">
        <div className="flex flex-col lg:flex-row justify-between items-center gap-10 mb-12">
           <div className="flex items-center gap-5">
              ...
              <div>
                 <h3 className="text-2xl font-black uppercase tracking-tighter text-slate-800">Vision Stratégique {data.month}</h3>
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Projection de clôture & Objectif mensuel</p>
              </div>
           </div>
           
           <div className="grid grid-cols-2 gap-6 w-full lg:w-auto">
              <div className="p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100">
                 <p className="text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest">Objectif National</p>
                 <p className="text-3xl font-black text-slate-900">{data.monthly.objective.toLocaleString()}</p>
              </div>
              <div className="p-6 bg-gradient-to-br from-orange-600 to-red-600 rounded-[2.5rem] text-white shadow-xl">
                 <p className="text-[10px] font-black text-white/50 uppercase mb-2 tracking-widest">Projection IA (PFM)</p>
                 <p className="text-3xl font-black">{stats.pfm.toLocaleString()}</p>
              </div>
           </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
           {data.regions.slice(0, 4).map((region, idx) => {
              ...
              return (
                <div key={idx} className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 group hover:bg-white hover:shadow-xl transition-all">
                   <div className="flex justify-between items-start mb-4">
                      <h4 className="text-[11px] font-black text-slate-800 uppercase tracking-tight">{region.name.replace('PRES ', '')}</h4>
                      <span className={`text-[9px] font-black px-2 py-1 rounded-full ${achievement >= 100 ? 'bg-emerald-100 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                         {achievement.toFixed(0)}%
                      </span>
                   </div>
                   <p className="text-3xl font-black text-slate-900 leading-none mb-4">{totalRegion.toLocaleString()}</p>
                   <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden">
                      <div className={`h-full ${achievement >= 100 ? 'bg-emerald-500' : 'bg-red-500'}`} style={{ width: `${Math.min(achievement, 100)}%` }}></div>
                   </div>
                </div>
              );
           })}
