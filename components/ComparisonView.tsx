...
import { ArrowLeftRight, TrendingUp, Calendar as CalendarIcon, Clock, ChevronDown, TrendingDown, Minus, Activity, ChevronLeft, ChevronRight, X, MapPin, Building2, Globe, Sparkles } from 'lucide-react';
...
// --- Sous-composant Calendrier Hypnotique ---
        ...
        <div className="relative bg-[#0f172a] rounded-[4rem] p-12 lg:p-16 text-white shadow-3xl overflow-hidden border border-white/5">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-12">
            <div className="flex items-center gap-10">
              ...
              <div>
                <h2 className="text-4xl lg:text-6xl font-black uppercase tracking-tighter leading-none mb-4 bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-white/40">Comparateur</h2>
            ...
            <div className="flex bg-white/5 p-1.5 rounded-[2rem] border border-white/10 backdrop-blur-2xl shadow-inner">
              {[
                ...
                <button 
                  ...
                  className={`px-8 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all relative ${
                    scale === s.id ? 'bg-white text-slate-900 shadow-xl' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  {s.label}
                </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="glass-card p-12 rounded-[3.5rem] shadow-3xl border border-white/40 relative overflow-hidden group">
           ...
           <div className="flex items-center gap-4 mb-10">
              ...
              <div>
                 <p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.3em]">MÉTRIQUE A</p>
                 <h4 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Référence Primaire</h4>
              </div>
           </div>
           ...
           <div className="space-y-6">
              <div className="flex gap-2 p-1.5 bg-slate-100/50 rounded-2xl">
                 {['national', 'region', 'site'].map(s => (
                   <button key={s} onClick={() => setScopeA(s as any)} className={`flex-1 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${scopeA === s ? 'bg-white text-blue-600 shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>
                      {s}
                   </button>
                 ))}
              </div>
              ...
              {scopeA !== 'national' && (
                 <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-500">
                    <div className="relative">
                       <MapPin size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-blue-400" />
                       <select value={regionA} onChange={(e) => setRegionA(e.target.value)} className="w-full pl-14 pr-8 py-5 bg-white border border-slate-200 rounded-[1.5rem] text-xs font-black uppercase text-slate-800 outline-none appearance-none focus:ring-4 ring-blue-50 shadow-inner">
                          {regions.map(r => <option key={r} value={r}>{r}</option>)}
                       </select>
                    </div>
                    {scopeA === 'site' && (
                       <div className="relative">
                          <Building2 size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-blue-400" />
                          <select value={siteA} onChange={(e) => setSiteA(e.target.value)} className="w-full pl-14 pr-8 py-5 bg-white border border-slate-200 rounded-[1.5rem] text-xs font-black uppercase text-slate-800 outline-none appearance-none focus:ring-4 ring-blue-50 shadow-inner">
              ...
              {scale === 'days' ? (
                <button onClick={() => setShowPicker('A')} className="w-full bg-slate-900 text-white rounded-[1.5rem] px-8 py-5 flex items-center justify-between group hover:bg-slate-800 transition-all shadow-xl">
                  <div className="flex items-center gap-4 text-xs font-black uppercase tracking-[0.2em]">
                    <CalendarIcon size={18} className="text-blue-400" /> {dateA}
                  </div>
                  <ChevronDown size={18} className="text-white/40 group-hover:text-white" />
                </button>
              ) : (
                <div className="flex gap-4">
                   <select value={yearA} onChange={(e) => setYearA(Number(e.target.value))} className="flex-1 bg-white border border-slate-200 rounded-[1.5rem] px-6 py-5 text-xs font-black uppercase outline-none focus:ring-4 ring-blue-50 shadow-inner">
                      {availableYears.map(y => <option key={y} value={y}>{y}</option>)}
                   </select>
                   {scale === 'months' && (
                      <select value={monthA} onChange={(e) => setMonthA(Number(e.target.value))} className="flex-1 bg-white border border-slate-200 rounded-[1.5rem] px-6 py-5 text-xs font-black uppercase outline-none focus:ring-4 ring-blue-50 shadow-inner">
           ...
           <div className="pt-10 mt-10 border-t border-slate-100 flex flex-col">
              <span className="text-7xl font-black text-slate-900 tracking-tighter leading-none">{comparisonResults.a.total.toLocaleString()}</span>
              <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.5em] mt-3">POCHES DE SANG RÉCOLTÉES</p>
           </div>
        </div>
        ...
        <div className="glass-card p-12 rounded-[3.5rem] shadow-3xl border border-white/40 relative overflow-hidden group">
           ...
           <div className="flex items-center gap-4 mb-10">
              ...
              <div>
                 <p className="text-[10px] font-black text-purple-500 uppercase tracking-[0.3em]">MÉTRIQUE B</p>
                 <h4 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Point de Comparaison</h4>
              </div>
           </div>
           ...
           <div className="space-y-6">
              <div className="flex gap-2 p-1.5 bg-slate-100/50 rounded-2xl">
                 {['national', 'region', 'site'].map(s => (
                   <button key={s} onClick={() => setScopeB(s as any)} className={`flex-1 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${scopeB === s ? 'bg-white text-purple-600 shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>
                      {s}
                   </button>
                 ))}
              </div>
              ...
              {scopeB !== 'national' && (
                 <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-500">
                    <div className="relative">
                       <MapPin size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-purple-400" />
                       <select value={regionB} onChange={(e) => setRegionB(e.target.value)} className="w-full pl-14 pr-8 py-5 bg-white border border-slate-200 rounded-[1.5rem] text-xs font-black uppercase text-slate-800 outline-none appearance-none focus:ring-4 ring-purple-50 shadow-inner">
                          {regions.map(r => <option key={r} value={r}>{r}</option>)}
                       </select>
                    </div>
                    {scopeB === 'site' && (
                       <div className="relative">
                          <Building2 size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-purple-400" />
                          <select value={siteB} onChange={(e) => setSiteB(e.target.value)} className="w-full pl-14 pr-8 py-5 bg-white border border-slate-200 rounded-[1.5rem] text-xs font-black uppercase text-slate-800 outline-none appearance-none focus:ring-4 ring-purple-50 shadow-inner">
              ...
              {scale === 'days' ? (
                <button onClick={() => setShowPicker('B')} className="w-full bg-slate-900 text-white rounded-[1.5rem] px-8 py-5 flex items-center justify-between group hover:bg-slate-800 transition-all shadow-xl">
                  <div className="flex items-center gap-4 text-xs font-black uppercase tracking-[0.2em]">
                    <CalendarIcon size={18} className="text-purple-400" /> {dateB}
                  </div>
                  <ChevronDown size={18} className="text-white/40 group-hover:text-white" />
                </button>
              ) : (
                <div className="flex gap-4">
                   <select value={yearB} onChange={(e) => setYearB(Number(e.target.value))} className="flex-1 bg-white border border-slate-200 rounded-[1.5rem] px-6 py-5 text-xs font-black uppercase outline-none focus:ring-4 ring-purple-50 shadow-inner">
                      {availableYears.map(y => <option key={y} value={y}>{y}</option>)}
                   </select>
                   {scale === 'months' && (
                      <select value={monthB} onChange={(e) => setMonthB(Number(e.target.value))} className="flex-1 bg-white border border-slate-200 rounded-[1.5rem] px-6 py-5 text-xs font-black uppercase outline-none focus:ring-4 ring-purple-50 shadow-inner">
                         {MONTHS_FR.map((m, i) => <option key={i} value={i}>{m}</option>)}
                      </select>
                   )}
                </div>
           ...
           <div className="pt-10 mt-10 border-t border-slate-100 flex flex-col">
              <span className="text-7xl font-black text-slate-900 tracking-tighter leading-none">{comparisonResults.b.total.toLocaleString()}</span>
              <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.5em] mt-3">POCHES DE SANG RÉCOLTÉES</p>
           </div>
        </div>
      </div>

      {/* RESULTS GRID HYPNOTIQUE */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
         <div className="bg-[#0f172a] rounded-[4rem] p-12 text-white flex flex-col justify-center items-center shadow-3xl relative overflow-hidden group">
            ...
            <div className="relative z-10 text-center">
              ...
              <div className="flex flex-col items-center gap-6">
                 <div className={`w-20 h-20 rounded-[2rem] flex items-center justify-center shadow-2xl ${comparisonResults.diff >= 0 ? 'bg-emerald-500' : 'bg-red-500'}`}>
                    {comparisonResults.diff >= 0 ? <TrendingUp size={40} /> : <TrendingDown size={40} />}
                 </div>
                 <div>
                    <p className="text-8xl font-black tracking-tighter leading-none mb-3">
                      {comparisonResults.diff > 0 ? '+' : ''}{comparisonResults.diff.toLocaleString()}
                    </p>
                    <div className={`inline-flex items-center gap-2 px-6 py-2 rounded-full font-black text-lg ${comparisonResults.perc >= 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
                       {comparisonResults.perc.toFixed(1)}%
                    </div>
                 </div>
              </div>
            </div>
         ...
         <div className="lg:col-span-2 glass-card rounded-[4rem] p-12 lg:p-16 shadow-3xl border border-white/40">
            <div className="flex items-center justify-between mb-12">
               <h3 className="text-2xl font-black uppercase tracking-tighter text-slate-800 flex items-center gap-6">
                  <Globe size={32} className="text-blue-600" /> Analyse Mixte
               </h3>
               <div className="flex gap-4">
            ...
            <div className="h-80">
               <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} barGap={20}>
                    ...
                    <Bar dataKey="A" fill={COLORS.blue} radius={[8, 8, 0, 0]} name="Période A" />
                    <Bar dataKey="B" fill="#e2e8f0" radius={[8, 8, 0, 0]} name="Période B" />
                  </BarChart>
               </ResponsiveContainer>
            </div>
         </div>
