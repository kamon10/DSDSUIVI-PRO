...
export const RecapView: React.FC<RecapViewProps> = ({ data, sites }) => {
  ...
  const handleWhatsAppMessage = (siteName: string, manager: string, phone: string, stats: any) => {
    if (!phone) return;
    const cleanPhone = phone.replace(/\s/g, '');
    const perc = stats.achievementGlobal;
    
    let intro = "";
    if (perc >= 100) {
      intro = `🌟 Félicitations ! Le site a atteint ses objectifs. Bravo à vous et à toute l'équipe pour ce résultat exceptionnel.`;
    } else if (perc >= 75) {
      intro = `👍 Bonne progression. Vous êtes sur la bonne voie, continuez vos efforts pour franchir la barre des 100% d'ici la fin du mois.`;
    } else {
    ...
    const message = `Bonjour ${manager},\n\n${intro}\n\n📍 Situation du site *${siteName}* au ${selectedDate} :\n- Collecte Fixe : ${stats.fixe}\n- Collecte Mobile : ${stats.mobile}\n- Total Jour : ${stats.totalJour}\n- Cumul Mensuel : ${stats.totalMois} poches\n- Taux d'atteinte : *${perc.toFixed(1)}%*\n\nRestons mobilisés pour sauver des vies.\nCordialement, DSD HEMO-STATS.`;
          ...
          <table className="w-full border-collapse border-2 border-slate-900 text-[10px] font-bold text-slate-950 leading-tight">
            <thead>
              <tr className="bg-slate-900 text-white">
                <th className="border border-slate-800 p-2 uppercase tracking-widest text-center text-[10px] w-[180px]">PRES / Région</th>
                <th className="border border-slate-800 p-2 uppercase tracking-widest text-left text-[11px]">Libellé Site</th>
                <th className="border border-slate-800 p-2 uppercase tracking-widest text-center text-[10px] w-[45px]">Fixe</th>
                <th className="border border-slate-800 p-2 uppercase tracking-widest text-center text-[10px] w-[45px]">Mob.</th>
                <th className="border border-slate-800 p-2 uppercase tracking-widest text-center text-[10px] w-[55px] bg-white/10">Jour</th>
                <th className="border border-slate-800 p-2 uppercase tracking-widest text-center text-[10px] w-[65px]">Mois</th>
                <th className="border border-slate-800 p-2 uppercase tracking-widest text-center text-[10px] w-[65px]">Objectif/M</th>
                <th className="border border-slate-800 p-2 uppercase tracking-widest text-center text-[10px] w-[55px]">Taux/M</th>
                {!exporting && <th className="border border-slate-800 p-2 uppercase tracking-widest text-center text-[10px] w-[40px] bg-slate-800">Msg</th>}
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <React.Fragment key={rIdx}>
                    {region.sites.map((site, sIdx) => {
                      ...
                      return (
                        <tr key={`${rIdx}-${sIdx}`} style={{ backgroundColor: regionColor }} className="hover:brightness-95 transition-all">
                          {sIdx === 0 && (
                            <td rowSpan={region.sites.length + 1} className="border border-slate-800 p-2 text-center align-top font-black uppercase text-[11px] text-slate-950 w-[180px]" style={{ backgroundColor: regionColor }}>
                          ...
                          {!exporting && (
                            <td className="border border-slate-800 p-1 text-center">
                              {siteInfo?.phone ? (
                                <button 
                                  onClick={() => handleWhatsAppMessage(site.name, siteInfo.manager, siteInfo.phone, site)}
                                  className="w-7 h-7 bg-emerald-500 text-white rounded-lg flex items-center justify-center hover:bg-emerald-600 transition-all shadow-sm"
                    ...
                    <tr style={{ backgroundColor: regionColor }} className="font-black brightness-90">
                      ...
                      {!exporting && <td className="border border-slate-800 p-1.5"></td>}
                    </tr>
                  </React.Fragment>
                );
              })}
