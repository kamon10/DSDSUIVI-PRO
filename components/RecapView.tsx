...
export const RecapView: React.FC<RecapViewProps> = ({ data }) => {
  ...
  // 1. CALCUL DES TOTAUX NATIONAUX BRUTS (LA VÉRITÉ DU FICHIER)
  const grandTotals = useMemo(() => {
    ...
    const totalJourRec = data.dailyHistory.find(h => h.date === selectedDate);
    
    const fixed = totalJourRec?.stats.fixed || 0;
    const mobile = totalJourRec?.stats.mobile || 0;
    const totalJour = totalJourRec?.stats.realized || (fixed + mobile);

    const totalMois = relevantHistory.reduce((acc, h) => acc + h.stats.realized, 0);
    const totalMoisFixe = relevantHistory.reduce((acc, h) => acc + h.stats.fixed, 0);
    const totalMoisMobile = relevantHistory.reduce((acc, h) => acc + h.stats.mobile, 0);

    const objMens = data.regions.reduce((acc, r) => 
      acc + r.sites.reduce((sAcc, s) => sAcc + s.objMensuel, 0), 0
    );
    ...
    return { 
      ...
      totalMoisMobile, 
      objMens, 
      achievementGlobal: objMens > 0 ? (totalMois / objMens) * 100 : 0 
    };
  }, [data, selectedDate]);

  // 2. CALCUL DES DONNÉES PAR RÉGION ET SITES
  const formattedData = useMemo(() => {
    ...
    return data.regions.map(region => {
      const regionSites = region.sites.map(site => {
        const dayRec = data.dailyHistory.find(h => h.date === selectedDate);
        const siteDailyEntries = dayRec?.sites.filter(s => {
          const matched = getSiteByInput(s.name);
          return matched && matched.name === site.name;
        }) || [];
        
        const fixe = siteDailyEntries.reduce((acc, s) => acc + (s.fixe || 0), 0);
        const mobile = siteDailyEntries.reduce((acc, s) => acc + (s.mobile || 0), 0);
        const totalJour = siteDailyEntries.reduce((acc, s) => acc + (s.total || 0), 0);

        const totalMois = data.dailyHistory
          .filter(h => {
            const [hD, hM, hY] = h.date.split('/').map(Number);
            return hY === selY && hM === selM && hD <= selD;
          })
          .reduce((acc, h) => {
            const entriesForSite = h.sites.filter(siteH => {
              const matched = getSiteByInput(siteH.name);
              return matched && matched.name === site.name;
            });
            return acc + entriesForSite.reduce((dayAcc, s) => dayAcc + (s.total || 0), 0);
          }, 0);
        ...
        return {
          ...
          totalJour,
          totalMois,
          achievementGlobal: site.objMensuel > 0 ? (totalMois / site.objMensuel) * 100 : 0
        };
  ...
  }, [data.regions, data.dailyHistory, selectedDate]);

  // 3. DÉTECTION DES ÉCARTS (Sites non identifiés dans SITES_DATA)
  const adjustments = useMemo(() => {
    if (!grandTotals) return null;
    const sumSitesJour = formattedData.reduce((acc, r) => acc + r.totalJourPres, 0);
    const sumSitesMois = formattedData.reduce((acc, r) => acc + r.totalMoisPres, 0);
    const sumSitesFixe = formattedData.reduce((acc, r) => acc + r.fixePres, 0);
    const sumSitesMobile = formattedData.reduce((acc, r) => acc + r.mobilePres, 0);

    const diffJour = grandTotals.totalJour - sumSitesJour;
    const diffMois = grandTotals.totalMois - sumSitesMois;
    const diffFixe = grandTotals.fixed - sumSitesFixe;
    const diffMobile = grandTotals.mobile - sumSitesMobile;

    if (diffJour === 0 && diffMois === 0) return null;

    return { diffJour, diffMois, diffFixe, diffMobile };
  }, [formattedData, grandTotals]);
  ...
  const handleExport = async (type: 'image' | 'pdf') => {
    ...
    try {
      ...
      const filename = `RECAP_CNTS_${selectedDate.replace(/\//g, '-')}`;
      if (type === 'image') {
      ...
      <div className="overflow-x-auto rounded-[2.5rem] shadow-2xl bg-white border border-slate-100">
        <div ref={recapRef} className="min-w-[850px] p-6 bg-white text-slate-900" style={{ width: '100%', maxWidth: '900px', margin: '0 auto' }}>
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            ...
            <div className="border-2 border-slate-900 p-3 flex flex-col items-center justify-center bg-orange-50/30">
               <div className="flex items-center gap-2 mb-1">
                 ...
                 <span className="text-[9px] font-black uppercase tracking-[0.2em] text-orange-400">CUMUL MENSUEL</span>
               </div>
            ...
            <div className="border-2 border-slate-900 p-3 flex flex-col items-center justify-center bg-blue-50/30">
               <div className="flex items-center gap-2 mb-1">
                 ...
                 <span className="text-[9px] font-black uppercase tracking-[0.2em] text-blue-500">OBJECTIF NATIONAL</span>
               </div>
          ...
          <table className="w-full border-collapse border-2 border-slate-900 text-[10px] font-bold text-slate-950 leading-tight">
            <thead>
              <tr className="bg-slate-900 text-white">
                ...
                <th className="border border-slate-800 p-2 uppercase tracking-widest text-center text-[10px] w-[70px]">Cumul M.</th>
            ...
            <tbody>
                ...
                return (
                  <React.Fragment key={rIdx}>
                    ...
                    <tr style={{ backgroundColor: regionColor }} className="font-black brightness-90">
                      <td className="border border-slate-800 p-1.5 uppercase text-right text-slate-950 text-[10px] h-7 pr-3 leading-none italic">TOTAL {region.name}</td>
              ...
              {adjustments && (
                <tr className="bg-slate-100 italic">
                  <td colSpan={2} className="border border-slate-800 p-2 text-right uppercase text-[9px] font-black text-slate-500 pr-4">AUTRES SITES / AJUSTEMENTS DE SAISIE</td>
                  <td className="border border-slate-800 p-1.5 text-center text-[10px] text-slate-500">{adjustments.diffFixe.toLocaleString()}</td>
                  <td className="border border-slate-800 p-1.5 text-center text-[10px] text-slate-500">{adjustments.diffMobile.toLocaleString()}</td>
                  <td className="border border-slate-800 p-1.5 text-center bg-white/40 text-[11px] font-black text-slate-500">{adjustments.diffJour.toLocaleString()}</td>
                  <td className="border border-slate-800 p-1.5 text-center text-[10px] text-slate-500">{adjustments.diffMois.toLocaleString()}</td>
                  <td colSpan={2} className="border border-slate-800 p-1.5 text-center bg-black/5 text-[9px] text-slate-400 uppercase font-bold">Inclus dans Total National</td>
                </tr>
              )}
            </tbody>
            <tfoot className="bg-slate-900 text-white font-black">
              <tr>
                ...
                <td className="border border-slate-800 p-3 text-center text-emerald-400 text-[18px]">{grandTotals.totalMois.toLocaleString()}</td>
          ...
          <div className="mt-4 flex justify-between items-center opacity-70 italic border-t-2 border-slate-100 pt-3">
            ...
            <p className="text-[9px] font-black uppercase tracking-widest text-slate-900">HEMO-STATS SUIVI • {selectedDate}</p>
          </div>
        </div>
      </div>
    </div>
